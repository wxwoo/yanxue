...
MCTSLIMIT = 100
SEARCHDEPTH = 10
COMPARE_ROUND = 50 # for each side, total round *2
PROCESS = 10
...
def MCTSmove(state, player, timeIterations, knowledge):
    input = ''
    for i in range(8):
        for j in range(8):
            input += str(state.board[i, j]) + ' '
...
def cmp_subProcess(processid, queue, st_state, aiIteration):
    c_state = GameState()
    cmpRound = int(COMPARE_ROUND / PROCESS)
    AI1MCTS = CNNMCTS(aiIteration, processid)
    AI1Win = 0
    AI2Win = 0
    ...
    for i in range(cmpRound):
        ...
        while not c_state.isTerminal():
            ...
            if len(c_state.getValidMoves(-1)) > 0:
                # c_state.makeMove(MCTSmove(c_state, -1, MCTSLIMIT, 0), -1)
                c_state.makeMove(priorityMove(c_state, -1), -1)
                # c_state.makeMove(mobilityMove(c_state, -1), -1)
                turn += 1
        ...
        if winner == 1:
            ...
            AI2Win += 1
        if DISPLAY_INFO == 1:
            print(f'process {processid} with AI is black: {i} / {cmpRound} ; AI : opponent = {AI1Win} : {AI2Win}')
    ...
    for i in range(cmpRound):
        ...
        if winner == 1:
            ...
            AI1Win += 1
        if DISPLAY_INFO == 1:
            print(f'process {processid} with AI is white: {i} / {cmpRound} ; AI : opponent = {AI1Win} : {AI2Win}')
...
if __name__ == '__main__':
    for i in range(90,-1,-10):
        ...
        print(f'AI iteration:{i}')
        
        queue = mp.Manager().Queue(maxsize=PROCESS * 2)
        mp.spawn(cmp_subProcess, nprocs=PROCESS, args=(queue, st_state, i))
        ...
        print('AI is black:')
        print(f'    AI win:{AI1BlackWin}')
        print(f'    opponent win:{AI2WhiteWin}')
        print(f'    Draw:{COMPARE_ROUND - AI1BlackWin - AI2WhiteWin}')
        print('AI is white:')
        print(f'    AI win:{AI1WhiteWin}')
        print(f'    opponent win:{AI2BlackWin}')
